...
import { createEvent, updateEvent, getUsersMatchingEventType } from "@talentbank/firebase-config";
...
export default function CreateEventPage() {
  ...
  function isSectionFilled(id: string): boolean {
    switch (id) {
      ...
      case "image":        return !!form.imageFile;
      case "registration": return form.registrationForm.length > 0;
      case "badge":        return form.badgeMode === "upload" ? !!form.badgeImageFile : true;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const docRef = await createEvent(payload);

      if (form.badgeMode === "upload" && form.badgeImageFile) {
        const ext      = form.badgeImageFile.name.split(".").pop() ?? "png";
        const bRef     = ref(storage, `badges/${docRef.id}/custom_badge.${ext}`);
        await uploadBytes(bRef, form.badgeImageFile);
        const badgeUrl = await getDownloadURL(bRef);
        await updateEvent(docRef.id, { badgeImageUrl: badgeUrl });
      }

      // Fire-and-forget push notification to students whose interests match the event type
    ...
    <main className="min-h-screen bg-[#F7F4EE]">
      ...
      <nav className="bg-[#F7F4EE]/90 border-b border-[#E2DED6] backdrop-blur-xl sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3 text-sm">
          ...
          <span className="text-[#3A332C] font-bold">Create Event</span>
        </div>
                ...
                {(["physical", "online"] as const).map((t) => (
                  <button key={t} type="button"
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition border-2
                      ${form.locationType === t ? "bg-[#E8923C]/10 border-amber-400/50 text-[#E8923C]" : "bg-[#F7F4EE] border-transparent text-[#3A332C]/50 hover:bg-[#E2DED6]"}`}
                  >
            ...
            <div className="space-y-3">
              <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-[#E2DED6] rounded-2xl cursor-pointer hover:border-amber-400/40 hover:bg-[#E8923C]/5 transition group relative overflow-hidden">
                {form.imagePreviewUrl ? (
                  <>
                    <img src={form.imagePreviewUrl} alt="Preview" className="absolute inset-0 w-full h-full object-cover" />
              ...
              {form.imageFile && (
                <button type="button"
                  onClick={() => setForm((f) => ({ ...f, imageFile: null, imagePreviewUrl: "" }))}
            ...
            {form.badgeMode === "design" ? (
              ...
              <div className="space-y-4">
                ...
                <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-[#E2DED6] rounded-2xl cursor-pointer hover:border-amber-400/40 hover:bg-[#E8923C]/5 transition group relative overflow-hidden">
                  {form.badgeImagePreviewUrl ? (
                    <>
                      <img src={form.badgeImagePreviewUrl} alt="Badge preview" className="absolute inset-0 w-full h-full object-contain p-6" />
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2 text-[#3A332C]text-sm font-semibold">
                ...
                {form.badgeImageFile && (
                  <button type="button"
                    onClick={() => setForm((f) => ({ ...f, badgeImageFile: null, badgeImagePreviewUrl: "" }))}
                    className="flex items-center gap-1.5 text-xs text-red-400/70 hover:text-red-400 transition"
                  >
